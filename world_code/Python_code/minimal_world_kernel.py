"""Minimal executable kernel: full lightcone + deterministic CxO update rule.
...
No observables are embedded in the kernel.
"""
...
def load_world(path: str) -> World:
    ...
    init_state = {str(k): _parse_cxo(v) for k, v in raw["init_state"].items()}
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Minimal full-lightcone CxO kernel runner.")
