"""Generate deterministic fine-structure test cases using the world_code kernel.
...
Outputs:
1) JSON dataset with preconditioning and measurement photon-coupling telemetry,
...
CHECKPOINT_NAME = "fine_structure_20_cases.checkpoint.json"
PARTIAL_SUMMARY_NAME = "fine_structure_20_cases.partial.json"
CASE_DIR_NAME = "cases"
...
def _seed_phase_buckets(world: object, position_node_ids: Sequence[str]) -> List[int]:
    return [_phase_bucket_8(world.states[nid][7]) for nid in position_node_ids]
...
def _step_coupling_row(
    world: object,
    position_node_ids: Sequence[str],
    prev_buckets: List[int],
    min_pos: int,
    max_pos: int,
    left_pos: int,
    right_pos: int,
    ...
    cur_buckets = [_phase_bucket_8(world.states[nid][7]) for nid in position_node_ids]
    ...
    if overlap_min <= overlap_max:
        ...
        for idx in range(start_idx, end_idx + 1):
            phase_jump_sum += _phase_jump_mod8(prev_buckets[idx], cur_buckets[idx])

    prev_buckets[:] = cur_buckets

    coupling_rate = (float(phase_jump_sum) / float(overlap_count * 8)) if overlap_count else 0.0
    return {
        "global_tick": int(world.tick),
        "overlap_started": bool(world.tick >= overlap_start_tick),
        "overlap_node_count": int(overlap_count),
        "phase_jump_sum": int(phase_jump_sum),
...
def _simulate_case(
    ...
    prev_buckets = _seed_phase_buckets(world, position_node_ids)
    ...
    post_overlap_rates_measurement: List[float] = []
    post_overlap_rates_all: List[float] = []
    for t in range(1, cfg.steps + 1):
        ...
        if row["overlap_started"]:
            post_overlap_rates_measurement.append(row["photon_coupling_rate"])

    for row in preconditioning_rows:
        if row["overlap_started"]:
            post_overlap_rates_all.append(row["photon_coupling_rate"])
    post_overlap_rates_all.extend(post_overlap_rates_measurement)

    mean_post_measurement = (
        sum(post_overlap_rates_measurement) / len(post_overlap_rates_measurement)
        if post_overlap_rates_measurement
        else 0.0
    )
    max_post_measurement = max(post_overlap_rates_measurement) if post_overlap_rates_measurement else 0.0
    mean_post_all = (
        sum(post_overlap_rates_all) / len(post_overlap_rates_all)
        if post_overlap_rates_all
        else 0.0
    )
    max_post_all = max(post_overlap_rates_all) if post_overlap_rates_all else 0.0
    ...
    return {
        ...
        "overlap_start_tick": overlap_start_tick,
        "summary": {
            "mean_coupling_rate_post_overlap_measurement": mean_post_measurement,
            "max_coupling_rate_post_overlap_measurement": max_post_measurement,
            "mean_coupling_rate_post_overlap_all": mean_post_all,
            "max_coupling_rate_post_overlap_all": max_post_all,
        },
...
def _simulate_case_worker(task: Tuple[CaseConfig, int, bool, str, bool, bool]) -> Dict[str, object]:
    ...
    if include_cold_baseline and preconditioning_ticks > 0:
        ...
        cold_mean = cold_baseline["summary"]["mean_coupling_rate_post_overlap_measurement"]
        warm_mean = case_payload["summary"]["mean_coupling_rate_post_overlap_measurement"]
        case_payload["cold_baseline_summary"] = cold_baseline["summary"]
        case_payload["summary"]["delta_mean_vs_cold_measurement"] = warm_mean - cold_mean
    return case_payload
...
def _build_payload(
    ...
    return {
        "schema_version": "fine_structure_20_cases_v3",
...
def _write_checkpoint(
    ...
    checkpoint = {
        "schema_version": "fine_structure_20_cases_checkpoint_v1",
...
def _write_partial_summary(
    ...
    payload = {
        "schema_version": "fine_structure_20_cases_partial_v1",
        "updated_at_utc": datetime.now(UTC).isoformat(),
        "settings": settings,
...
def execute_cases_incremental(
    ...
    if workers == 1:
        for task in tasks:
            ...
            mean_rate = case_payload["summary"]["mean_coupling_rate_post_overlap_measurement"]
            print(f"[{done:02d}/{total}] completed {cfg.case_id} | mean post-overlap={mean_rate:.6f}")
    else:
        with concurrent.futures.ProcessPoolExecutor(max_workers=workers) as executor:
...
def main() -> None:
    ...
    parser.add_argument(
        "--resume",
        action="store_true",
        help="Resume from fine_structure_20_cases.checkpoint.json if available.",
    )
    parser.add_argument(
        "--disable-fast-step",
        action="store_true",
    ...
    if not args.execute:
        raise SystemExit(
            ...
            "Example: python world_code/Python_code/generate_fine_structure_20_cases.py "
