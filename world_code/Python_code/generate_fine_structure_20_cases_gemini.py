...
def execute_and_save_cases(out_dir: Path) -> None:
    """Orchestrates the generation, execution, and incremental saving of cases."""
    ...
    print(f"Output directory established at: {out_dir}")

    # === PHASE 2: Case Generation and File Setup ===
    print("\n--- PHASE 2: GENERATING CASE CONFIGURATIONS ---")
    cases = _make_case_grid()
    print(f"Generated {len(cases)} case configurations.")
    ...
    # Initialize the CSV with headers so we can append to it during the loop
    ...
    for i, cfg in enumerate(cases):
        # 3a. Run the simulation
        case_obj = _simulate_case(cfg)
        payload_cases.append(case_obj)

        # 3b. Output results to the CSV immediately (Incremental Save)
        ...
        # 3c. Output console progress after each case
        mean_rate = case_obj["summary"]["mean_coupling_rate_post_overlap"]
        print(f"[{i+1:02d}/{len(cases)}] Completed {cfg.case_id} | Mean Post-Overlap Rate: {mean_rate:.6f}")

    # === PHASE 4: Final JSON Payload ===
    print("\n--- PHASE 4: FINALIZING ARTIFACTS ---")
    payload = {
    ...
    # Write the completed master JSON summary
    json_path.write_text(json.dumps(payload, indent=2), encoding="utf-8")
    print(f"Successfully wrote aggregated JSON to: {json_path}")
...
def main() -> None:
    ...
    # Long-running task is now orchestrated incrementally
    execute_and_save_cases(Path(args.output_dir))
