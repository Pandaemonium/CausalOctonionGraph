"""Smoke test for build_v3_s2880_particle_interaction_catalog_v1."""
...
from cog_v3.calc import build_v3_s2880_particle_interaction_catalog_v1 as mod
...
def main() -> None:
    ...
    if payload["schema_version"] != "v3_s2880_particle_interaction_catalog_v1":
        raise AssertionError("schema mismatch")
    if int(payload["summary"]["element_count"]) != 2880:
        raise AssertionError("expected 2880 catalog rows")
    if int(payload["summary"]["class_count"]) <= 0:
        raise AssertionError("class_count must be positive")
    for p in (OUT_ELEM_CSV, OUT_CLASS_CSV, OUT_JSON, OUT_MD):
        if not p.exists():
    ...
    print("ok: test_v3_s2880_particle_interaction_catalog_v1")
